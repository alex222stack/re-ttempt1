...
const KIMI_K3_REASONING_EFFORT = 'low';
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    if (error.response) {
      console.error('NIM API status:', error.response.status);
      console.error('NIM API response:', JSON.stringify(error.response.data, null, 2));
    }

    // Forward NVIDIA's actual error message back to the client
    const nimError = error.response?.data?.error || error.response?.data;

    res.status(error.response?.status || 500).json({
      error: {
        message: nimError?.message || error.message || 'Internal server error',
        type: nimError?.type || 'invalid_request_error',
        code: error.response?.status || 500,
        details: nimError || null
      }
    });
  }
});
